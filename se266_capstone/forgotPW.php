...
<html lang="en">
    ...
    <body>
        ...
        $un_form = "<form method='post' id='name-form'>"
            ...
            <br/><br/>
            <label style='color:white'>Barber:</label> &nbsp;<input type='radio' name='table' value='barber_accounts' required> 
            <label style='color:white'>User:</label> &nbsp;<input type='radio' name='table' value='user_accounts' required> 
            <br/><br/>"
                . "<br/>"
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($table == 'barber_accounts') {
                $_SESSION['id'] = 'BarberID';
                $un = "barberUserName";
                $_SESSION['un'] = "barberUserName";
            ...
            $stmt = $db->prepare("SELECT security_question, $acc_id FROM $table WHERE $un = '$uname';");
            echo "SELECT security_question, $acc_id FROM $table WHERE $un = '$uname';";
            $un_form = "";
        ...
        //COMPARE ANSWER TO DATABASE DATA. SECURE
        if (isset($_POST['answer_submit'])) {
            //Encrypt and Add Salt
            $tbl = $_SESSION['table'];
            $acc_id = $_SESSION['id'];
            $id_data = $_SESSION["acc_id"];
            $sq_answer = Sha1(filter_input(INPUT_POST, 'sq_answer_text')) . $salt;
            $answer_stmt = $db->prepare("SELECT * FROM $tbl WHERE sq_answer = '$sq_answer' AND $acc_id = '$id_data';");
            echo "SELECT * FROM $tbl WHERE sq_answer = '$sq_answer' AND $acc_id = '$id_data';";
            $un_form = "";
        ...
        if (isset($_POST['pw_answer_submit'])) {
            ...
            $pw_update_stmt = $db->prepare("UPDATE $tbl SET pw = '$new_pw' WHERE $the_id='$id' AND $un_data = '$un';");

            //EXECUTE SQL AND CHECK FOR RETURNED ROWS
            if ($pw_update_stmt->execute() && $pw_update_stmt->rowCount() > 0) {
                $message = "<span style='color:red;'>*" . ' ' . "</span>Your Password Has Successfully Been Updated!<span style='color:red;'>" . ' ' . "*</span>"
                        . "<br/>"
                        . "<br/>"
    ...
    <center>
        <div class="div-form">
            ...
            <br/>
            <br/>
            <a href="UserLogin.php" class="a">Login</a><label class="symbols">&nbsp;&#9900;&nbsp;</label>
